...
import { escapeHtml } from './textSecurity.js';
...
export async function sendPasswordReset(email, resetUrl) {
  return getResend().emails.send({
    ...
    to: email,
    ...replyTo,
    subject: 'Resetează-ți parola QRPhotoDrop',
    html: shell('Resetare parolă', `
      <p>Am primit o cerere de resetare a parolei pentru contul tău QRPhotoDrop.</p>
      <p>Apasă butonul de mai jos pentru a seta o parolă nouă. Linkul expiră în scurt timp.</p>
      ${button(resetUrl, 'Setează o parolă nouă')}
      <p style="color:#7a789a;font-size:14px">Dacă nu ai cerut tu resetarea, ignoră acest email — parola rămâne neschimbată.</p>
    `),
  });
}
...
export async function sendContactForm({ firstName, lastName, email, phone, eventType, message }) {
  const safe = {
    ...
    eventType: escapeHtml(eventType || 'N/A'),
    message: escapeHtml(message).replace(/\r?\n/g, '<br>'),
  };
  return getResend().emails.send({
    from: FROM,
    to: process.env.ADMIN_EMAIL,
    subject: `Mesaj Contact — ${String(firstName || '').replace(/[\r\n]/g, ' ')} ${String(lastName || '').replace(/[\r\n]/g, ' ')}`.slice(0, 180),
    html: `
      <h2>Mesaj nou de contact</h2>
      <p><strong>Nume:</strong> ${safe.firstName} ${safe.lastName}</p>
      <p><strong>Email:</strong> ${safe.email}</p>
      <p><strong>Telefon:</strong> ${safe.phone}</p>
      <p><strong>Tip eveniment:</strong> ${safe.eventType}</p>
      <hr />
      <p>${safe.message}</p>
    `,
  });
