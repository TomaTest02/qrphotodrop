...
export async function cleanUserStorage(admin, userId, logLabel) {
  ...
  //
  // `deactivated_by_suspension: false` e OBLIGATORIU: ștergerea contului anulează
  // motivul anterior de oprire. Altfel, dacă contul era suspendat (marcaj = true) și
  // cleanup-ul R2 eșuează, o reactivare ulterioară ar reporni evenimentul — deși
  // fișierele lui au fost deja (parțial) șterse din R2.
  const { error: statusError } = await admin
    .from('events')
    .update({ status: 'inactive', deactivated_by_suspension: false })
    .in('id', eventIds);
