...
export async function generateAndUploadArchive(eventId, userEmail) {
  ...
  // Buffer-ul complet e gata abia la evenimentul 'end' — finalize() singur nu
  // garantează că toate chunk-urile s-au scurs (altfel arhiva iese truncată).
  const archiveBuffer = new Promise((resolve, reject) => {
  ...
  // 6. Upload ZIP to R2 (așteptăm stream-ul complet)
  const zipBuffer = await archiveBuffer;
